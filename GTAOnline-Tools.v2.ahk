...
if !A_IsAdmin {
	...
	if (ErrorLevel != 0) {
		MsgBox("This script requires administrator privileges! Please run it again with the correct privileges.", s_toolTipTitle . " | Error!", "Iconx")
	}
	ExitApp()
}
...
global
	...
	doSuspend := MsgBox("Confirm Port Block to initiate a GTA 5 Solo Public Session?`n Blocking Ports: " . s_soloPublicSessionBlockPorts, s_toolTipTitle . " | Confirm!", "YesNo Icon!")
	if (doSuspend = "Yes") {
		...
		RunWait('netsh advfirewall firewall add rule name="' s_soloPublicSessionPortBlockRule '" dir=out protocol=udp localport="' s_soloPublicSessionBlockPorts '" action=block enable=yes', , "Hide")
...
global
	...
	doSuspend := MsgBox("Confirm Process Suspend to initiate a GTA 5 Solo Public Session?`n Required Tool: PsSuspend.exe`n Process name: " . s_procExeName, s_toolTipTitle . " | Confirm!", "YesNo Icon!")
	if (doSuspend = "Yes") {
...
global
	...
	RunWait('netsh advfirewall firewall add rule name="' s_noSaveRule '" dir=out action=block remoteip="' s_progressSaveIP '"', , "Hide")
...
global
	...
	RunWait('netsh advfirewall firewall add rule name="' s_exeBlockRule '" dir=out program="' s_procExePath '" action=block enable=yes', , "Hide")
...
UpdateToolTip() {
	global s_hideToolTipActive, s_hideToolTipLevel, s_tempShowToolTip, s_isExeFound, s_procExeName, s_toolTipTitle, s_tooltipPrefix, s_afkActive, s_autoClickActive, s_autoClickTimer, s_soloPublicSesPortBlockActive, s_soloPublicSesPortBlockTimer, s_soloPublicSuspendTimer, s_soloPublicSesProcSuspendActive, s_soloPublicSesProcSuspendTimer, s_noSaveRuleActive, s_exeBlockRuleActive
	...
	if (shouldShowDetails) {
		...
		if (CanShowThisFeature(ToolFeatures.AUTO_CLICK)) {
			if (s_autoClickActive) {
				featuresTT.Push(s_tooltipPrefix . "AUTO-CLICK                    [Ctrl+F6]  - ✔️    -  [TIMER: " . s_autoClickTimer . "s]")
				showDisableHkColum := true
			} else {
		...
		if (CanShowThisFeature(ToolFeatures.SOLO_PUBLIC_SESSION_PORT)) {
			if (s_soloPublicSesPortBlockActive) {
				featuresTT.Push(s_tooltipPrefix . "SOLO PUBLIC SESSION - PORT    [Ctrl+F7]  - ✔️    -  [WAIT:  " . s_soloPublicSesPortBlockTimer . "s]")
				showDisableHkColum := true
			} else {
		...
		if (CanShowThisFeature(ToolFeatures.SOLO_PUBLIC_SESSION_SUSPEND)) {
			if (s_soloPublicSesProcSuspendActive) {
				featuresTT.Push(s_tooltipPrefix . "SOLO PUBLIC SESSION - SUSPEND [Ctrl+F8]  - ✔️    -  [WAIT:  " . s_soloPublicSesProcSuspendTimer . "s]")
				showDisableHkColum := true
			} else {
		...
		if (s_hideToolTipLevel = 2) {
			activeFeatureFlags := [s_afkActive, s_autoClickActive, s_soloPublicSesPortBlockActive, s_soloPublicSesProcSuspendActive, s_noSaveRuleActive, s_exeBlockRuleActive]
...
AppExit(*) {
	global exeVarsTimerFunc, s_procExeName, s_suspendExe, s_noSaveRule, s_exeBlockRule, s_soloPublicSessionPortBlockRule
